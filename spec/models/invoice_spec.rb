...
RSpec.describe Invoice, type: :model do
  ...
  describe "#total_discounted_revenue" do
    it "only applies the discount to the item that meets the quantity threshold" do
      @merchant1 = Merchant.create!(name: 'Hair Care')
      ...
      # needs fixing is applying both discounts
      expect(@invoice_1.total_revenue).to eq(200)
      expect(Invoice.total_discounted_revenue(@invoice_1)).to eq(190)
    end
    ...
    it "applies the discount to both items that meet the quantity threshold" do
      ...
      expect(Invoice.total_discounted_revenue(@invoice_1)).to eq(216)
    ...
    it "applies only the highest discount that the invoice qualifies for" do
      ...
      @ii_1 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_1.id, quantity: 9, unit_price: 10, status: 2)
      @ii_11 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_8.id, quantity: 1, unit_price: 10, status: 1)
      # needs fixing is applying all discounts
      expect(Invoice.total_discounted_revenue(@invoice_1)).to eq(75)
    end
    ...
    it "applies the discounts only from the appropriate merchant offering the item" do
      ...
      @discount_1 = @merchant1.bulk_discounts.create!(name: "10 percent off total", percentage: 10, quantity_threshold: 0)
      @discount_2 = @merchant2.bulk_discounts.create!(name: "20 percent off total", percentage: 20, quantity_threshold: 0)
      @item_1 = Item.create!(name: "Shampoo", description: "This washes your hair", unit_price: 10, merchant_id: @merchant1.id, status: 1)
      ...
      # needs fixing is applyign discounts from both merchants
      expect(@invoice_1.total_revenue).to eq(400)
      expect(Invoice.total_discounted_revenue(@invoice_1)).to eq(380)
    end
    ...
    it "does not apply the discount if the quantity threshold is not met" do
      ...
      @ii_11 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_8.id, quantity: 1, unit_price: 10, status: 1)
      #works as intended
      expect(Invoice.total_discounted_revenue(@invoice_1)).to eq(100)
    end
